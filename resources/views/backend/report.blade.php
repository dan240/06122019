	...
	<table id="example" class="display" style="width:100%">
		...
		<tbody>
			...
			<tr id="tr_{{$row['id']}}">
				...
				<td>{{ @$row['user']['firstname'] }} {{ @$row['user']['lastname'] }}  @if(!empty($row['user']['usertype']) && $row['user']['usertype'] == 1) - <b>Company</b> @else - <b>Investor</b> @endif  <br> {{ @$row['user']['email'] }}</td>
				
				@if(!empty($row['company_detail']))
					
					<td>{{ @$row['company_detail']['firstname'] }} {{ @$row['company_detail']['lastname'] }} ( Company ) <br> {{ @$row['company_detail']['email'] }}</td>
				@else
					
					<td>{{ @$row['investor_detail']['firstname'] }} {{ @$row['investor_detail']['lastname'] }} ( Investor ) <br> {{ @$row['investor_detail']['email'] }}</td>
				@endif
				...
				<td class="action">
					...
					<a class="btn btn-danger btn-xs" data-original-title="Remove" data-toggle="tooltip" data-placement="top" href="javascript:void(0)" onclick="deleteReportAbuse('{{$row['id']}}')">
...
$('#example').DataTable();
	...
	function deleteReportAbuse(id)
	{
		$.ajax({
			method:"POST",
			url:"{{ url('Admin/deleteReportAbuse')}}",
