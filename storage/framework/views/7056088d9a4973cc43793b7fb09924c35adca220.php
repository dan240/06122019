...
<div class="container-fluid">
	<div class="row" style="padding:10px;">
        ...
        <div class="col-md-6 text-right">
          <a class="btn btn-primary" href="<?php echo e(url('Admin/addUserForm')); ?>">Add User</a>
        </div>
     </div>
	...
	<table id="example" class="display" style="width:100%">
		...
		<tbody>
			...
			<tr>
				<td><?php echo e($i); ?></td>
				<td>
					<?php echo e(@$row['firstname']); ?> <?php echo e(@$row['lastname']); ?> 
					<?php if($row['usertype']=='1') { ?>
						<b> - Company</b>
					<?php }elseif($row['usertype']=='2') {?>
						<b> - Investor</b>
					<?php } ?>
					<br> <?php echo e(@$row['email']); ?>
				...
				<td>
					...
					<span id="see_status_<?php echo e($u_id); ?>"><?= ucfirst(@$row['see_contacts']) ?></span>
					
					<button class="btn btn-success btn-sm" id="chng_see_<?php echo e($u_id); ?>"
						onclick='changeContactVisablity("<?php echo e($u_id); ?>")' data-current="<?php echo e($current); ?>">
						<i class="fa fa-repeat"></i>
					</button>
				</td>
				<td>
					<?php echo e(@ucfirst($row['subscription_plan'])); ?>
				...
				<td>Send Message - <?php echo e(@$row['msg_sent']); ?> <br> Meeting Request - <?php echo e(@$row['meeting_req']); ?> <br> Profile Card Visits - <?php echo e($row['visit_count']); ?></td>
				<td>
					
						
					<?php $message_meeting_approval = $row['message_meeting_approval']; ?>
					<span id="message_meeting_approval_<?php echo e($u_id); ?>">
					...
					<button class="btn btn-success btn-sm" id="chng_message_meeting_approval_<?php echo e($u_id); ?>"
						onclick='changeMessageMeetingApproval("<?php echo e($_u_id); ?>")' data-current="<?php echo e($_current_message_meeting_approval); ?>">
						<i class="fa fa-repeat"></i>
					</button>
				
				</td>
				<td><?php echo e(@$row['userip']); ?></td>
				...
				<td>
					...
					<span id="prof_status_<?php echo e($u_id); ?>">
					...
					<button class="btn btn-success btn-sm" id="chng_prof_<?php echo e($u_id); ?>"
						onclick='changeProfessionalStatus("<?php echo e($_u_id); ?>")' data-current="<?php echo e($_current_prof); ?>">
						<i class="fa fa-repeat"></i>
					</button>
				...
				<td>
					...
					<span id="public_status_<?php echo e($u_id); ?>">
						<?php if($approve == '1' ){ ?>
							Public
						<?php }else{ ?>
							Not Public
					...
					<button class="btn btn-success btn-sm" id="chng_public_<?php echo e($u_id); ?>"
						onclick='changePublicStatus("<?php echo e($_u_id); ?>","<?php echo e($_userType); ?>")' data-current="<?php echo e($_current); ?>">
						<i class="fa fa-repeat"></i>
					</button>
				</td>
				...
				<?php if($row['status']==2 || $row['status']==3){ ?>
					<a class="btn btn-success btn-xs" data-original-title="Approve" data-toggle="tooltip" data-placement="top" href="javascript:void(0)" onclick="ApprovedProfile('<?php echo e($row['id']); ?>',this)"> 
	                  <i class="fa fa-check"></i> 
	                </a>
            	<?php }elseif($row['status']==1){ ?>
	                <!-- <a class="btn btn-warning btn-xs" data-original-title="Suspend" data-toggle="tooltip" data-placement="top" href="javascript:void(0)" onclick="InactiveUser('<?php echo e($row['id']); ?>',this)"> 
	                  <i class="fa fa-times-circle-o"></i> 
	                </a> -->
            	<?php } ?>
	                <a class="btn btn-danger btn-xs" data-original-title="Remove" data-toggle="tooltip" data-placement="top" href="javascript:void(0)" onclick="DeleteUser('<?php echo e($row['id']); ?>')"> 
	                  <i class="fa fa-ban"></i> 
	                </a>
	                <a class="btn btn-info btn-xs" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="javascript:void(0)" onclick="editUser('<?php echo e($row['id']); ?>')"> 
...
<script>
	$(document).ready(function(){
		...
		$('body').on('click','.featuredAction', function (){
			...
			$.ajax({
				method:"POST",
				url:"<?php echo e(url('Admin/ChangeFeatured')); ?>",
				data:{"_token":"<?php echo e(csrf_token()); ?>",userId:userId, action: action, userType: UserType},
	...
	function InactiveUser(id,obj)
	{
		$.ajax({
			method:"POST",
			url:"<?php echo e(url('Admin/InactiveUser')); ?>",
	...
	function changeContactVisablity(id){
		...
		$.ajax({
			method:"POST",
			url:"<?php echo e(url('Admin/changeVisablity')); ?>",
			data:{"_token":"<?php echo e(csrf_token()); ?>",id:id,current : current},
			success:function(response)
			{
				if(current == "not"){
	...
	function changePublicStatus(userId,userType){
		...
		$.ajax({
			method:"POST",
			url:"<?php echo e(url('Admin/changePublicStatus')); ?>",
			data:{"_token":"<?php echo e(csrf_token()); ?>", userid : userId, usertype : userType,current : current},
			success:function(response)
			{
				if(current == "1"){
	...
	function changeProfessionalStatus(userId){
		...
		$.ajax({
			method:"POST",
			url:"<?php echo e(url('Admin/changeProfStatus')); ?>",
			data:{"_token":"<?php echo e(csrf_token()); ?>", userid : userId, current : current},
			success:function(response)
			{
				if(current == "1"){
	...
	function changeMessageMeetingApproval(userId){
		...
		$.ajax({
			method:"POST",
			url:"<?php echo e(url('Admin/changeMessageMeetingApproval_')); ?>",
			data:{"_token":"<?php echo e(csrf_token()); ?>", userid : userId, current : current},
			success:function(response){
				if(current == "no"){
	...
	function ApprovedProfile(id,obj){
		$.ajax({
			method:"POST",
			url:"<?php echo e(url('Admin/ApprovedProfile')); ?>",
			data:{"_token":"<?php echo e(csrf_token()); ?>",id:id},
			success:function(response)
			{
	...
	{
		window.location.href="<?php echo e(url('Admin/editUser/')); ?>/"+id;

	}
	</script>
	
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/admin/web/londcap-app.com/public_html/resources/views/backend/user.blade.php ENDPATH**/ ?>